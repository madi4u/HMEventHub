...
const APP_ID = "eventhub"

const PUBLIC_PATHS = ['/login', '/reset-password', '/api/auth', '/api/webhooks']
...
export async function middleware(request: NextRequest) {
  ...
  const appBase = process.env.NEXT_PUBLIC_APP_URL ?? `https://eventhub.hundm.cloud`
  const callbackUrl = `${appBase}${request.nextUrl.pathname}${request.nextUrl.search}`

  if (!sessionCookie) {
    return NextResponse.redirect(
      `${AUTH_SERVICE}/login?callback=${encodeURIComponent(callbackUrl)}`
    )
  }
